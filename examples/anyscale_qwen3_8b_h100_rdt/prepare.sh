#!/bin/bash
# Bundle local sglang and ray source into _bundled/ so it gets uploaded via working_dir.
...
if [ ! -d "${RAY_DIR}/python/ray" ]; then
    echo "ERROR: ray not found at ${RAY_DIR}/python/ray"
    echo "Set RAY_DIR to the ray repo root, e.g.: RAY_DIR=../ray bash $0"
    exit 1
fi

mkdir -p _bundled/sglang_python _bundled/ray_python
